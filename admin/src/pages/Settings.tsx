...
const SettingsPage = () => {
  ...
  const {
    ...
    restoreSettingsMutationSuccess: () => {
      ...
      toggleNotification({
        message: formatMessage({id: getTranslation('page.settings.action.restore.confirmation.success')}),
        type: 'success',
      });
    },
  ...
  if (config.status !== 'success') {
    // TODO
    return formatMessage({id: getTranslation('page.settings.loading')});
  }
              ...
              <Flex gap={4} direction="column">
                <Box background="neutral0" padding={6} shadow="filterShadow" width="100%" hasRadius>
                  ...
                  <Grid.Root gap={4} marginTop={4} width="100%">
                    <Grid.Item xs={12}>
                      <Field.Root width="100%" hint={formatMessage({id: getTranslation('page.settings.form.cloud-name')})}>
                        <Field.Label htmlFor="cloud_name">
                          {formatMessage({id: getTranslation('page.settings.form.cloud-name')})}
                        </Field.Label>
                        <Field.Input value={values.cloud_name} name="cloud_name" onChange={onChange} />
                        <Field.Hint />
                      </Field.Root>
                    </Grid.Item>
                    <Grid.Item xs={12}>
                      <Field.Root width="100%" hint={formatMessage({id: getTranslation('page.settings.form.api-key')})}>
                        <Field.Label htmlFor="api_key">
                          {formatMessage({id: getTranslation('page.settings.form.api-key')})}
                        </Field.Label>
                        <Field.Input value={values.api_key} name="api_key" onChange={onChange} />
                        <Field.Hint />
                      </Field.Root>
                    </Grid.Item>
                  </Grid.Root>
                    ...
                    <ConfirmationDialog
                      Trigger={({ onClick }) => (
                        <Button
                          ...
                          onClick={onClick}
                        >
                          {formatMessage({id: getTranslation('page.settings.action.restore.label')})}
                        </Button>
                      )}
                      onConfirm={restoreSettingsMutation.mutate}
                      title={formatMessage({id: getTranslation('page.settings.action.restore.confirmation.header')})}
                      labelConfirm={formatMessage({id: getTranslation('page.settings.action.restore.confirmation.confirm')})}
                      iconConfirm={<ArrowClockwise />}>
                        {formatMessage({id: getTranslation('page.settings.action.restore.confirmation.description')})}
                    </ConfirmationDialog>
                  </Flex>
                </Box>
              </Flex>
